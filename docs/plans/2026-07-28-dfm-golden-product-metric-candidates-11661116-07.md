...
- STEP/Parasolid 产品 B-Rep、模具/浇排模型和模流仿真结果是三类不同输入，不能要求 NX
  产品几何 Backend 从单个产品 B-Rep 推导仿真结果；
- 报告与黄金产品实际输入必须通过文件版本、图纸版本和 SHA-256 对齐后，才能用于验收。

## 2. M2.6-B/C 候选优先指标：产品几何与出模性

这些项目适合 `STEP/Parasolid → NX Server → NX C++ Plugin → Measurement` 生产链路，
建议优先进入黄金产品追溯矩阵。表中的 P0/P1 是黄金产品业务优先级，不代表 M2.6-A/B/C
阶段编号，也不表示 P0 要在 M2.6-A 一次性完成。
...
M2.6-A 先严格使用当前冻结的 `injection.wall-draft@2.0.0`，只实现 ABS、mm、一个开模
方向、壁厚与拔模角的 NX STEP/Parasolid 最小闭环。M2.6-A 通过后，再在 M2.6-B 建立独立
压铸 Scope/RuleBinding，并按“一项指标一个完整闭环”逐步纳入以下候选 P0 项：
...
| `dc.process.projected_area.runner_overflow` | 浇道、渣包等浇排系统投影面积       | 浇排投影面积`cm²`         | 作为锁模力公式输入                                         | `650 cm²`                                         | 27     | 需要包含浇排系统的模具/铸件模型，单个产品 B-Rep 不足 |
...
这些内容有价值，但不宜作为“单个产品 B-Rep 的通用几何指标”。它们依赖模具设计、
...
| `dc.manufacturing.machining_allowance` | 各机加工区域余量是否足够？                   | 大平面`1.2 mm`；其他小平面 `0.8 mm`；孔径单边 `0.5～0.8 mm`                                  | 毛坯模型、成品模型、机加工区域映射        | 26     | 有两版模型时可做几何差分，单个成品 B-Rep 不足 |

## 5. 模流与凝固仿真候选指标

报告第 28–45 页来自模流/凝固仿真。它们不能通过 NX 对产品 STEP/Parasolid 做普通 B-Rep
Measurement 得到。建议登记到长期指标清单，但 M2.6-A NX 几何闭环不承诺计算；
...
产品基线；它们不是由 STEP/Parasolid 必然可靠读取的事实。
...
1. 当前同源黄金 STEP/Parasolid 与报告使用的产品版本是否完全一致？报告第 40–45 页多次提到“产品
